...
export default function DashboardPage() {
  ...
  const [recentTransactions, setRecentTransactions] = useState([
    { id: 1, date: '2025-01-30', station: 'ТРЦ "Глобус"', stationId: 'EV-CH-001', kwh: 25.5, amount: 216.75 },
    { id: 2, date: '2025-01-28', station: 'Паркінг ТРЦ', stationId: 'EV-CH-002', kwh: 18.2, amount: 163.80 },
    { id: 3, date: '2025-01-25', station: 'АЗС "ЕКО"', stationId: 'EV-CH-003', kwh: 42.0, amount: 378.00 },
  ]);
  ...
  function checkAuth() {
    ...
    try {
      ...
      // Load user-specific balance if available
      const savedBalance = localStorage.getItem(`balance_${parsedUser.id}`);
      if (savedBalance) {
        setStats(prev => ({ ...prev, balance: parseFloat(savedBalance) }));
      }
    } catch (e) {
      router.push('/auth/login');
    } finally {
  ...
  function handleTopUp() {
    const newBalance = stats.balance + topUpAmount;
    ...
    setSuccessMessage(`Баланс поповнено на ₴${topUpAmount}. Новий баланс: ₴${newBalance.toFixed(2)}`);
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
              ...
              <tbody>
                {recentTransactions.map((tx) => (
                  <tr key={tx.id} className="border-b border-gray-100 last:border-0">
                    ...
                    <td className="py-4 text-gray-500 font-mono text-sm">{tx.stationId}</td>
      ...
      {/* Top Up Modal */}
      <Modal isOpen={showTopUpModal} onClose={() => setShowTopUpModal(false)} title="Поповнити баланс">
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Сума поповнення</label>
            ...
            <input
              type="number"
              value={topUpAmount}
              onChange={(e) => setTopUpAmount(Math.max(50, parseInt(e.target.value) || 50))}
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500"
              min="50"
              step="10"
            />
            <p className="text-sm text-gray-500 mt-1">Мінімальна сума: ₴50</p>
          </div>
          
          <div className="bg-gray-50 rounded-lg p-4">
            <div className="flex justify-between text-sm">
              <span className="text-gray-500">Поточний баланс:</span>
              <span className="text-gray-800">₴{stats.balance.toFixed(2)}</span>
            </div>
            <div className="flex justify-between text-sm mt-1">
              <span className="text-gray-500">Поповнення:</span>
              <span className="text-green-600">+₴{topUpAmount}</span>
            </div>
            <div className="flex justify-between font-medium mt-2 pt-2 border-t">
              <span>Новий баланс:</span>
              <span className="text-green-600">₴{(stats.balance + topUpAmount).toFixed(2)}</span>
            </div>
          </div>
          ...
          <p className="text-xs text-gray-400 text-center">
            * Демо режим. В реальній системі тут буде інтеграція з платіжною системою.
          </p>
        </div>
      </Modal>
